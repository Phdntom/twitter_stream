...
with open("STATE_CODES.json") as fobj:
    STATE_CODES = json.load(fobj)
print STATE_CODES
...
def prep_JSON(state_dist, us_dist):
    '''
    '''
    total_count = 0
    json_dict = {}
    json_dict["country"] = "US"
    json_dict["countrydist"] = us_dist
    json_dict["states"] = {}

    for state in state_dist:
        name = STATE_CODES[state]
        #list_dist = []
#        json_dict["states"][state] = {"code":state,"dist":{} }
        #for key in state_dist[state]:
        #    list_dist.append( (key, state_dist[state][key] ) )
        json_dict["states"][name] = state_dist[state]
    """ 
        for label in state_dist[state]:
            json_dict["states"][state]["dist"][label] = state_dist[state][label]
            #json_dict["states"][state]["dist"][label]["value"] = \
            #                                           state_dist[state][label]
    """
    return json_dict

def get_bin_label(tweet_score):
    '''
    -5 to -3, 0
    -3 to -1, 1
    -1 to +1, 2
...
def main():
    ...
    '''
    state_dist = defaultdict( lambda: defaultdict(float))
    for tweet in candidates:
        state = get_state(tweet["user"]["location"])
        if state is not None:
            tweet_score = score_tweet(scores, tweet)
            key = get_bin_label(tweet_score)
            state_dist[state][key] += 1

    us_dist = defaultdict(float)
    for state in state_dist:
        for bin_label in state_dist[state]:
            us_dist[bin_label] += state_dist[state][bin_label]

    #for state in state_dist:
    #    print state, state_dist[state]

    print us_dist

    json_dict = prep_JSON(state_dist,us_dist)

    #print json_dict

    outname = "distribution_" + tweet_file[3:]
